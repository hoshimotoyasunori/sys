-- 既存のプロジェクトに対してタスクと成果物を強制的に作成
-- このマイグレーションは、フロントエンドでタスク・成果物が作成されない問題を解決するため
...
BEGIN
    ...
    LOOP
        ...
        -- 既にタスクと成果物が存在する場合は削除して再作成
        IF existing_tasks_count > 0 THEN
            DELETE FROM tasks WHERE project_id = project_record.id;
        END IF;
        
        IF existing_deliverables_count > 0 THEN
            DELETE FROM deliverables WHERE project_id = project_record.id;
        END IF;
